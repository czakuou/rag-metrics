...
def make_ragas_judge_llm() -> BaseRagasLLM:
    # ragas.evaluate(llm=...) only accepts BaseRagasLLM/LangChain objects, so ChatOpenAI is
    # used purely as an OpenAI-schema-compatible HTTP client — base_url points at the
    # LiteLLM proxy, not at OpenAI, so the provider stays swappable via litellm config.
    chat = ChatOpenAI(
