...
- **It is not explainable.** "We used LangChain" describes a dependency, not a
  design. A from-scratch ReAct loop is small enough that every line has a reason
  that can be stated and defended.
...
- Narrower framework-migration risk than LangChain/LlamaIndex — BAML's scope
  is limited to the LLM contract (prompt, schema, retry), so a breaking change
  in BAML can only ever touch that surface, not retrieval or orchestration
...
- BAML is itself a framework: a DSL, a code generator, and a runtime
  (`baml_client/`) this project does not control. It is younger and less
  battle-tested than LangChain, and a breaking change in the BAML CLI or
  runtime would still require a coordinated regeneration of `baml_client/`
  across the repo. The bet here is not "no framework risk" — it is that a
  framework scoped to one narrow concern (LLM I/O contracts) is a smaller bet
  than one that also owns orchestration, memory, and tool dispatch.
...
├── baml_client/          # generated — do not edit manually, see addendum below
│   ├── __init__.py
│   ├── types.py
│   └── ...               # async_client.py, sync_client.py, runtime.py, tracing.py,
│                          # parser.py, type_builder.py, and other BAML-generated
│                          # support modules — full set generated by `make generate`,
│                          # not hand-maintained, so not enumerated here
```

`baml_client/` is generated from `baml_src/` by running
`uvx baml-cli generate --from src/rag/agent/baml_src/` (the `generate` Makefile target).
...
## Addendum: `baml_client/` is committed, not gitignored (2026-06-30)

The original decision was to treat `baml_client/` as build output: exclude it from
version control via `.gitignore`, regenerate via `make generate` on install. In
practice the repo committed `baml_client/` from the start and `.gitignore` was never
updated to match — the two drifted instead of one being fixed to match the other.

**Decision: keep `baml_client/` committed, drop the `.gitignore` requirement.**
This repo's primary audience includes readers who clone it to read code, not to run
`make generate` first — a missing `baml_client/` would make `agent/pipeline.py` fail
to import with no code visible to explain why. Committing
the generated client also makes `git diff` show the actual effect of a `baml_src/`
change (schema, prompt, retry config) on the generated types/client, which is useful
review signal that a gitignored-and-regenerated file would hide.

**Consequence:** `baml_client/` must be regenerated and committed in the same PR as any
`baml_src/` change — `make generate` is a pre-commit step, not a post-clone step. A
stale `baml_client/` that no longer matches `baml_src/` is now possible (it wasn't,
when the client was gitignored and always freshly generated) and would not be caught
automatically; if this becomes a real problem, add a CI check that re-runs
`make generate` and fails on a resulting diff.
